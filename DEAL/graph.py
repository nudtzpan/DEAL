...
class NeighborFinder:
    ...
    def re_get_temporal_neighbor(self, src_idx_l, cut_time_l, num_neighbor=20, flag=0):
        """
        Params
        ------
        src_idx_l: List[int]
        ...
        for i, (src_idx, cut_time) in enumerate(zip(src_idx_l, cut_time_l)):
            ...
            time_delta = cut_time - ngh_ts
            sampling_weight = np.exp(- flag[i] * self.bias * time_delta)
            sampling_weight = sampling_weight / sampling_weight.sum()  # normalize
            sampled_idx = np.sort(np.random.choice(np.arange(len(ngh_idx)), num_neighbor, replace=True, p=sampling_weight))
    ...
    def re_find_k_hop(self, k, src_idx_l, cut_time_l, num_neighbors, flag):
        """Sampling the k-hop sub graph in tree struture
        """
        ...
        batch = len(src_idx_l)
        layer_i = 0
        x, z = self.re_get_temporal_neighbor(src_idx_l, cut_time_l, num_neighbors[layer_i], flag)
        node_records = [x]
        t_records = [z]
        for layer_i in range(1, k):
            ...
            ngh_node_est = ngh_node_est.flatten()
            ngh_t_est = ngh_t_est.flatten()
            flag = np.repeat(np.expand_dims(flag, 1), num_neighbors[0], axis=1).reshape((-1,))
            out_ngh_node_batch, out_ngh_t_batch = self.re_get_temporal_neighbor(ngh_node_est, ngh_t_est, num_neighbors[layer_i], flag)
            out_ngh_node_batch = out_ngh_node_batch.reshape(batch, -1)
            out_ngh_t_batch = out_ngh_t_batch.reshape(batch, -1)
