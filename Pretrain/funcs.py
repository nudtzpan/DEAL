...
def set_up_logger(args, sys_argv):
    ...
    log_root = './Pretrain/log/'
    if not os.path.exists(log_root):
        os.mkdir(log_root)
        logger.info('Create directory {}'.format(log_root))
    file_path = './Pretrain/log/{}.log'.format(runtime_id) #TODO: improve log name
    ...
    checkpoint_root = './Pretrain/saved_checkpoints/'
    checkpoint_dir = checkpoint_root + 'pretrain_' + runtime_id + '/'
    pretrain_model_root = './PretrainEmbs/'
    if not os.path.exists(pretrain_model_root):
        os.mkdir(pretrain_model_root)
        logger.info('Create directory {}'.format(pretrain_model_root))
    ...
    pretrain_model_path = pretrain_model_root + 'pretrain_paras_'+args.data+'.pth'

    return logger, get_checkpoint_path, pretrain_model_path
